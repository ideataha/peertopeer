...
class Node(object):
    ...
    def sendmessage(self,code,message,receiver):
        ...
        # return "send %s to %s with code" % (message,receiver,code)
    ...
    def run(self):
        #if self.id == "segment-original":
            #self.setsegment()
            #print("id : %s  ---> lat : %f  &  long: %f" % (self.id,self.lat,self.long))
        if len(self.messagebuffer) != 0:
            self.readmessage()
...
#print("%d  |  %d" % (len(Global.xsegments),len(Global.ysegments)))
temp=0
while temp < len(Global.latsegments):
    ...
    #supernodelist[len(supernodelist)-1].id = ""
    if temp != 0:
...
#Global.nodelist.append(Node(False,1,26,37))
temp=0
while temp < numberofpeers:
    node = Node(False,Global.createid(Global),Global.createlat(Global),Global.createlong(Global))
    '''tp=node.id.split("-")
    node.left = Global.supernodelist[int(tp[0])-1].id'''
    Global.nodelist.append(node)
    temp=temp+1
...
while tempo < cycletorun and fflag:
    ...
    print("\n\n")
    print("list of unlinearized peers for cycle: %d" % Global.counter)
    counter = 1
